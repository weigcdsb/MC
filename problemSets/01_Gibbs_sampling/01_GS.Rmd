...
According to https://www.webmd.com/first-aid/normal-body-temperature, the normal body temperature of the adult ranging from 97F to 99F, I set the prior parameters as $\mu_0 = 98, \tau^2_0=1, \nu_0=4, \sigma^2_0=0.5$. Draw 5000 samples, with initial values be $\theta^{(1)}=\bar{\boldsymbol{x}}$ and $\tilde{\sigma}^{2(1)}=1/s^2$. The code are as follows. In the code, I further denote $\phi=(\theta, \tilde{\sigma}^2)'$
...
From the above analysis, the 95% HPD for the posterior mean is [98.121, 98.369] and 95% HPD for the posterior variance is [0.415, 0.678]. Since 98.6 is not in 95% HPD of the mean, the normal body temperature is not 98.6.
